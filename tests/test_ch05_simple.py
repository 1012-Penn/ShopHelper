...
async def test_complaint_no_llm_two_actions():
    ...
    assert [(type(m).__name__, m.content) for m in out["messages"]] == [("AIMessage", COMPLAINT_REPLY)]
...
async def test_log_persists_and_emits_actions():
    ...
    msgs = [HumanMessage(content="我要投诉", id="u1"),
            AIMessage(content=COMPLAINT_REPLY)]
    rec = Rec()
    out = await node(_state(session_id=9, user_message="我要投诉", final_reply=COMPLAINT_REPLY,
                            messages=msgs, turn_user_msg_id="u1",
    ...
    assert store.appended[0][1] == [{"role": "user", "content": "我要投诉"},
...
async def test_log_self_check_pool_on_refusal():
    ...
    await node(_state(session_id=3, user_message="量子力学怎么退货", final_reply=reply,
                      messages=[HumanMessage(content="量子力学怎么退货", id="u1"),
                                AIMessage(content=reply)],
                      turn_user_msg_id="u1", suggested_actions=[], trace=[]))
