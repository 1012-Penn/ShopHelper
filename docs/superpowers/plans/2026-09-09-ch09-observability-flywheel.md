...
### Task 2：检索快照与正式 evidence confidence 闸
...
### Task 3：统一 FlywheelService、标准化查重和 review_queue 数据层
...
### Task 4：把三个入口接入飞轮、反馈 API 和审核知识回写
...
### Task 5：评估脚本、eval_runs 趋势接口和意图 token 汇总
...
### Task 6：后台管理页与聊天 👎 后端接线（Vibe Coding 前端例外）
